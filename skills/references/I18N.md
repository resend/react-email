...
React Email supports three popular i18n libraries: next-intl, react-i18next, and react-intl.
...
<Text>
  Order date: {dateFormatter.format(new Date())}
</Text>
```
...
<Text>
  Total: {currencyFormatter.format(99.99)}
</Text>
...
<Text>
  Items: {numberFormatter.format(1234567)}
</Text>
```
...
```tsx
formatMessage({ id: 'itemCount' }, { count: items.length })
```

### Interpolation

All libraries support variable interpolation:

```json
{
  "greeting": "Hi {name}, welcome!",
  "order": "Your order #{orderId} has shipped"
}
```
...
```tsx
// Example: Get locale from user object
const userLocale = user.preferredLocale || user.country || 'en';

await resend.emails.send({
  react: <WelcomeEmail name={user.name} locale={userLocale} />
});
```

### 9. Subject Line Translation
