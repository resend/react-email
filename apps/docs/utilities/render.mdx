...
With `useRenderingOptions(uniqueRenderId?: string)` you can get the render options from inside your template.
It is optional but recommended to include a `uniqueRenderId` to prevent conflicts between parallel calls to render.

For example, if you want to only include a part of your template when rendering as plain text, you could create your template as:

```jsx email.jsx
import * as React from 'react';
import { Html, Button, Hr, Text, useRenderingOptions } from "@react-email/components";

export function MyTemplate({uniqueRenderId}) {
...
And then you would generate and pass the `uniqueRenderId` to the `render` function.
In this example, we use the crypto library to generate a UUID.
...
<ResponseField name="uniqueRenderId" type="string">
  Used to ensure that parallel calls to render do not conflict when using useRenderingOptions
</ResponseField>
