...
1. Replace all imports of individual packages `@react-email/body`, `@react-email/render`, `@react-email/tailwind`, etc. with `react-email`.
2. Replace all imports of `@react-email/components` with `react-email`.
