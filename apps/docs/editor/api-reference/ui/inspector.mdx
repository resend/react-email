...
Meant for inspecting and changing things related to the entire editor's content. That is, [EmailTheming](/editor/api-reference/theming-api), page-level settings, and other document-wide concerns.

It shows up when the editor is not focused on a specific node or text selection, but not when the editor is unfocused.
