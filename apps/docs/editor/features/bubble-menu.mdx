...
Add `BubbleMenu.Root` as a child of `EditorProvider` to get a fully-featured formatting toolbar.
...
When `BubbleMenu.Root` has no children, it renders the default text formatting toolbar automatically.
...
<>
  ...
  <BubbleMenu.Root hideWhenActiveNodes={['button']} hideWhenActiveMarks={['link']} />
