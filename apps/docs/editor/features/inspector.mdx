...
Add [EmailTheming](/editor/api-reference/theming-api) and render `Inspector.Root` next to the editor content.
...
`Inspector.Root` requires the [EmailTheming](/editor/api-reference/theming-api) extension. Without it, the
...
| Context parameter | Description |
| -- | -- |
| `marks` | A map of active text marks like `bold`, `italic`, `underline`, `strike`, and `code`. |
| `toggleMark(mark)` | Toggles a text mark on the current selection. |
| `alignment` | The current alignment of the parent text block. |
| `setAlignment(value)` | Updates the alignment of the parent text block. |
| `linkHref` | The current link URL when the selection is inside a link. |
| `linkColor` | The resolved color for the active link. |
| `setLinkColor(color)` | Updates the color of the active link. |
| `isLinkActive` | Whether the current text selection is inside a link. |
| `getStyle(prop)` | Reads a resolved style value from the parent text block. |
| `setStyle(prop, value)` | Updates a style on the parent text block. |
| `presetColors` | Colors collected from the current document that can be reused in custom controls. |
...
<CardGroup cols={2}>
  ...
  <Card title="Composed Inspector" icon="code" href="https://editor-examples.react.email/#inspector-composed">
