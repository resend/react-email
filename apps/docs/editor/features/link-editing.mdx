...
Add `LinkBubbleMenu.Default` to enable an inline link editing toolbar:
...
- **URL display** — Shows the current link URL
...
When combining the link bubble menu with `BubbleMenu.Default`, use `excludeMarks` to prevent
the text bubble menu from appearing when a link is focused:

```tsx
import { BubbleMenu, LinkBubbleMenu } from '@react-email/editor/ui';

<EditorProvider extensions={extensions} content={content}>
  <BubbleMenu.Default excludeMarks={['link']} />
  <LinkBubbleMenu.Default />
</EditorProvider>
```

Without `excludeMarks`, both menus would try to show at the same time when a link is selected.
