...
<Note>
  This is a guide intended at users of [yarn mordern](https://yarnpkg.com/getting-started/qa#why-should-you-upgrade-to-yarn-modern) 
  for those who are using [yarn classic](https://www.npmjs.com/package/yarn) (`1.x`) you can do something similar to the 
  [pnpm guide](/getting-started/monorepos-setup/pnpm).
</Note>
...
and in there setup a new `package.json` and do not forget to add this to the `workspaces` of your monorepo's `package.json`.
...
Install React Email to the `transactional` workspace along with React Email's unstyled components:
...
<Card title="React Email + Turborepo + yarn example" icon="arrow-up-right-from-square" href="https://github.com/resend/react-email-turborepo-yarn-example">
  See the full source code using `nodeLinker: node-modules`
</Card>
