...
export const buildPreview = async ({ dir }: Args) => {
  ...
  try {
    ...
    if (hasReactEmailDirectory) {
      ...
      if (isUpToDate) {
        ...
        startBuildPreview(packageManager);
...
const startBuildPreview = (packageManager: string) => {
  shell.exec(`${packageManager} run build`, { async: true });
};
