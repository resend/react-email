...
const mapUserWorkspaceDependencies = async (
  userDependencies: Record<string, string>,
) => {
  const resultingDependencies = { ...userDependencies };

  const projectRunningPreviewWorkspaceDirectory =
    await findWorkspaceDir(CURRENT_PATH);
  ...
  for (const dependencyProject of worksapceDependencies) {
    if (
      dependencyProject.manifest.name &&
      dependencyProject.manifest.name in resultingDependencies
    ) {
      resultingDependencies[
        dependencyProject.manifest.name
      ] = `file:${dependencyProject.dir}`;
    }
  }

  const remainingWorkspaceDependencies = Object.keys(
    resultingDependencies,
  ).filter((key) => resultingDependencies[key]?.startsWith('workspace'));
  if (remainingWorkspaceDependencies.length > 0) {
    throw new Error(
      `Sync PKG error: could not resolve specific paths to map workspace dependencies into for dependencies:\n${remainingWorkspaceDependencies
        .map((dep) => `- ${dep}`)
        .join('\n')}`,
    );
  }
...
export const syncPkg = async () => {
  ...
  if (clientPkg.dependencies) {
    for (const dep of NON_NEGOTIABLE_PREVIEW_CLIENT_DEPENDENCIES) {
      const clientDependency = clientPkg.dependencies[dep];
      if (typeof clientDependency !== 'undefined') {
        dependencies[dep] = clientDependency;
      }
    }
  } else {
    throw new Error(
      'Sync PKG Error: an unexpected error has happenned, this is mostly a safe guard than an actual error, but the dependencies on the preview client seems to not be defined, please create an issue on this',
    );
  }
