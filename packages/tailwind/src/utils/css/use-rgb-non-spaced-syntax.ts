...
const regex = /rgb\(\s*(\d+)\s*(\d+)\s*(\d+)(?:\s*\/\s*([\d%.]+))?\s*\)/;
 ...
 */
export const useRgbNonSpacedSyntax = (cssDeclarationValue: string) => {
  return cssDeclarationValue.replace(regex, (_match, r, g, b, a) => {
