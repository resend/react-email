...
.box {
  width: var(--width);
    ...
    resolveAllCssVariables(root);
    expect(generate(root)).toMatchInlineSnapshot(`":root{--width: 100px}.box{width:100px}"`);
  });
  ...
  it('works for variables across different CSS layers', () => {
    ...
    resolveAllCssVariables(root);
    expect(generate(root)).toMatchInlineSnapshot(`"@layer base{:root{--width: 100px}}@layer utilities{.box{width:100px}}"`);
  });
  ...
  it('works with multiple variables in the same declaration', () => {
    ...
    resolveAllCssVariables(root);
    expect(generate(root)).toMatchInlineSnapshot(`":root{--top: 101px;--bottom: 102px;--right: 103px;--left: 104px}.box{margin:101px 103px 102px 104px}"`);
  });
...
`);
    resolveAllCssVariables(root);
    expect(generate(root)).toMatchInlineSnapshot(`".box{--width: 200px;width:200px}@media (min-width:1280px){.xl\\\\:bg-green-500{--tw-bg-opacity: 1;background-color:rgb(34 197 94/1)}}"`);
  });
...
@layer utilities {
  .sm\\\\:bg-blue-300 {
    ...
    resolveAllCssVariables(root);
    expect(generate(root)).toMatchInlineSnapshot(`"@layer theme{:root{--color-blue-300: blue}}@layer utilities{.sm\\\\:bg-blue-300{@media (width>=40rem){background-color:blue}}}"`);
  });
  ...
  margin: var(--undefined-margin, 10px 20px);
    ...
    resolveAllCssVariables(root);
    expect(generate(root)).toMatchInlineSnapshot(`".box{width:150px;height:200px;margin:10px 20px}"`);
  });
...
.box {
  ...
  height: var(--undefined-height, var(--also-undefined, 250px));
    ...
    resolveAllCssVariables(root);
    expect(generate(root)).toMatchInlineSnapshot(`":root{--fallback-width: 300px}.box{width:300px;height:250px}"`);
  });
...
.box {
  ...
  background: var(--bg-color, rgb(var(--r, 255), var(--g, 0), var(--b, 0)));
    ...
    resolveAllCssVariables(root);
    expect(generate(root)).toMatchInlineSnapshot(`":root{--primary: blue;--secondary: red;--fallback: green;--size: 20px}.box{color:blue;width:20px;border:1px solid black;--r: 100;--b: 10;background:rgb(100,0,10)}"`);
  });
