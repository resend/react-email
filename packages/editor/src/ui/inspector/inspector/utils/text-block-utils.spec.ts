...
describe('applyMarkToggles', () => {
  ...
  /**
   * Builds a fake chain object that records which toggle methods were called.
   * Each toggle method returns the chain itself for continued chaining.
   */
  ...
  it('toggles multiple marks at once', () => {
    ...
    // These should NOT be toggled since they weren't requested and aren't active
  ...
  it('handles mixed active/requested state correctly', () => {
    ...
    // Request bold (already active) and underline (not active), drop italic (active)
    applyMarkToggles(chain, ['bold', 'underline'], state);
    expect(chain._called).not.toContain('toggleBold'); // already matches
    expect(chain._called).toContain('toggleItalic'); // needs to toggle off
    expect(chain._called).toContain('toggleUnderline'); // needs to toggle on
  });
