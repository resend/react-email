...
/**
 * Checks whether a section has any non-empty values set, by looking up
 * its properties in the style object and attributes.
 */
...
/**
 * Shared hook that manages the expand/collapse state of inspector sections.
 *
 * It unifies the logic previously duplicated between `InspectorLocal`
 * (which used `addedSections` Set) and `InspectorText` (which used a
 * manual `expandedSections` record).
 *
 * A section is considered visible when:
 * - It is in the `expandedSections` list (always visible), OR
 * - It has been manually added via `addSection()`, OR
 * - It already has non-empty values in `styleObject` / `attrs`
 */
export function useCollapsibleSections({
  ...
  // Reset added sections when the config changes (e.g. different node selected)
