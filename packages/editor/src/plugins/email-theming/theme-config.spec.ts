...
describe('isThemeConfig', () => {
  ...
  it('returns true for object with extends and styles', () => {
    expect(isThemeConfig({ extends: 'basic', styles: { body: { backgroundColor: '#ff0000' } } })).toBe(true);
  });
});
...
describe('themeStylesToPanelOverrides', () => {
  it('overrides body background on basic theme', () => {
    ...
    const result = themeStylesToPanelOverrides({ body: { backgroundColor: '#ff0000' } }, base);
    const bodyGroup = result.find((g) => g.id === 'body');
    const bgInput = bodyGroup?.inputs.find((i) => i.prop === 'backgroundColor' && i.classReference === 'body');
  ...
  it('overrides button backgroundColor and borderRadius as px string', () => {
    ...
    const bgInput = buttonGroup?.inputs.find((i) => i.prop === 'backgroundColor' && i.classReference === 'button');
    const radiusInput = buttonGroup?.inputs.find((i) => i.prop === 'borderRadius' && i.classReference === 'button');
  ...
  it('overrides link color', () => {
    const base = EDITOR_THEMES.basic;
    const result = themeStylesToPanelOverrides({ link: { color: '#abcdef' } }, base);
    const linkGroup = result.find((g) => g.id === 'link');
    const colorInput = linkGroup?.inputs.find((i) => i.prop === 'color' && i.classReference === 'link');
  ...
  it('preserves unmodified container width of 600', () => {
    ...
    const widthInput = containerGroup?.inputs.find((i) => i.prop === 'width' && i.classReference === 'container');
    expect(widthInput?.value).toBe(600);
  });

  it('works with minimal theme base', () => {
    const base = EDITOR_THEMES.minimal;
    const result = themeStylesToPanelOverrides({ link: { color: '#ff0000' } }, base);
    const linkGroup = result.find((g) => g.id === 'link');
    const colorInput = linkGroup?.inputs.find((i) => i.prop === 'color' && i.classReference === 'link');
  ...
  it('adds new input when property does not exist in base panel (h1 color)', () => {
    const base = EDITOR_THEMES.basic;
    const result = themeStylesToPanelOverrides({ h1: { color: '#ff0000' } }, base);
    const h1Group = result.find((g) => g.id === 'h1');
    const colorInput = h1Group?.inputs.find((i) => i.prop === 'color' && i.classReference === 'h1');
    expect(colorInput?.value).toBe('#ff0000');
  });

  it('adds new input with parsed unit when property does not exist (h1 fontSize as "32px")', () => {
    const base = EDITOR_THEMES.basic;
    const result = themeStylesToPanelOverrides({ h1: { fontSize: '32px' } }, base);
    const h1Group = result.find((g) => g.id === 'h1');
    const fontSizeInput = h1Group?.inputs.find((i) => i.prop === 'fontSize' && i.classReference === 'h1');
  ...
  it('does not mutate base panels', () => {
    const base = EDITOR_THEMES.basic;
    const originalBodyBg = base.find((g) => g.id === 'body')?.inputs.find((i) => i.prop === 'backgroundColor')?.value;
    themeStylesToPanelOverrides({ body: { backgroundColor: '#mutated' } }, base);
    const afterBodyBg = base.find((g) => g.id === 'body')?.inputs.find((i) => i.prop === 'backgroundColor')?.value;
...
describe('createTheme', () => {
  it('returns ThemeConfig without extends', () => {
    ...
    expect(result).toEqual({ styles: { body: { backgroundColor: '#ff0000' } } });
...
describe('extendTheme', () => {
  it('returns ThemeConfig with extends set', () => {
    const result = extendTheme('basic', { body: { backgroundColor: '#ff0000' } });
    expect(result).toEqual({ extends: 'basic', styles: { body: { backgroundColor: '#ff0000' } } });
  });
