...
export const listenersForImage = (
  callback: (node: TSESTree.Literal | TSESTree.TemplateElement, sourceType: ImageSourceType) => void,
): RuleListener => {
  return {
    "JSXOpeningElement:matches([name.name='Img'], [name.name='img']) JSXAttribute[name.name='src'] :matches(Literal, TemplateElement)":  (
      ...
      const value = node.type === TSESTree.AST_NODE_TYPES.TemplateElement ? node.value.cooked : node.value;
