...
EMAILS_DIR_ABSOLUTE_PATH=${path.resolve(dirname, '../emails')}
USER_PROJECT_LOCATION=${path.resolve(dirname, '..')}
...
const emailsDirectoryPath = path.resolve(dirname, '../emails');
...
for (let i = 0; i < directoryCount; i++) {
  ...
  for (let j = 0; j < emailsPerDirectoryCount; j++) {
    await fs.writeFile(path.join(directoryPath, `file-${i}-${j}.tsx`), emptyEmail, 'utf8');
  }
}
