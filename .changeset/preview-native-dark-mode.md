...
Turning the color inversion off also no longer leaves a white background on an email that never painted one. The inversion gives `<body>` the colors it already renders at so it has something to invert, and those were being restored on the way out as though the email had authored them.
